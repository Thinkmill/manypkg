...
export default makeCheck<ErrorType>({
  ...
  validate: workspace => {
    ...
    if (peerDeps) {
      for (let depName in peerDeps) {
        ...
        } else if (!semver.gte(devDeps[depName], peerDeps[depName])) {
          errors.push({
            type: "INVALID_DEV_AND_PEER_DEPENDENCY_RELATIONSHIP",
            workspace,
            dependencyName: depName,
  ...
  print: error => {
    if (error.devVersion === null) {
      return `${error.workspace.name} has a peerDependency on ${
        error.dependencyName
      } but it is not also specified in devDependencies, please add it there.`;
    }
    return `${error.workspace.name} has a peerDependency on ${
      error.dependencyName
    } but the range specified in devDependency is not greator than or equal to the range specified in peerDependencies`;
  }
});
